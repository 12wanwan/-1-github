...
const ParticleText = ({
  ...
  useEffect(() => {
    ...
    const drawParticle = particle => {
      ...
      if (glow && !reducedMotion && size > 2.1 && particle.seed < 0.4) {
      ...
      if (size <= 2.1) {
        ctx.fillRect(particle.x - r, particle.y - r, size, size);
        return;
      }
    ...
    const sampleText = async () => {
      ...
      const maxParticles = Math.max(600, Math.min(3000, Math.floor((width * height) / 140)));
